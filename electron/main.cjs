...
if (PORTABLE_USER_DIR) {
  ...
  process.env.LITTLE_PRINCE_AGENT_USER_DIR ||= PORTABLE_USER_DIR
...
const STARTUP_STEPS = [
  ...
  { id: 'core', label: '启动本地核心', detail: '加载 小王子 Agent runtime' },
...
function validatePackagedNativeModules() {
  ...
  if (issues.length) {
    throw new Error(`Packaged native module integrity check failed:\n${issues.join('\n')}\nPlease close 小王子 Agent and reinstall it with the official installer.`)
  }
}
...
function createTerminalStreamWindow(payload = {}) {
  const { title = '小王子 Agent Terminal Stream', stream_id = 'default' } = payload
  const cleanTitle = String(title || '小王子 Agent Terminal Stream').slice(0, 120)
...
app.whenReady().then(async () => {
  ...
  try {
    ...
    console.error('[main] Failed to load 小王子 Agent UI', err?.stack || err?.message || err)
    emitStartupProgress({ id: 'interface', status: 'error', error: true, message: `进入界面失败: ${err.message}` })
    dialog.showErrorBox('Startup failed', `Unable to load the 小王子 Agent interface:\n${err.message}`)
    app.quit()
    return
  }
  setupTray()
