...
Function BailongmaNormalizeInstallDir
  ...
  ${if} $R0 != "小王子 Agent"
    StrCpy $INSTDIR "$INSTDIR\小王子 Agent"
  ${endIf}
FunctionEnd
...
Function BailongmaFindForeignInstallRootItem
  ...
  bailongmaScanInstallRoot:
    ...
    StrCmp $R2 ".." bailongmaScanInstallRootNext
    StrCmp $R2 "小王子 Agent.exe" bailongmaScanInstallRootNext
    StrCmp $R2 "Uninstall 小王子 Agent.exe" bailongmaScanInstallRootNext
...
Function BailongmaValidateInstallDir
  ...
  ${GetFileName} "$INSTDIR" $R0
  ${if} $R0 != "小王子 Agent"
    MessageBox MB_ICONSTOP|MB_OK "Please install 小王子 Agent into its own folder, for example:$\r$\n$\r$\nD:\小王子 Agent$\r$\nD:\Apps\小王子 Agent$\r$\n$\r$\nCurrent path:$\r$\n$INSTDIR"
    Abort
  ${endIf}

  Call BailongmaFindForeignInstallRootItem
  ${if} $R3 != "0"
    MessageBox MB_ICONSTOP|MB_OK "The selected 小王子 Agent install folder already contains non-小王子 Agent content:$\r$\n$\r$\n$INSTDIR\$R3$\r$\n$\r$\nTo protect your files and other software, choose an empty folder or a folder used only by 小王子 Agent."
    Abort
  ${endIf}
...
Function BailongmaInstallDirSafetyPageCreate
  ...
  ${NSD_CreateLabel} 0 0 100% 24u "小王子 Agent will be installed into this application-owned folder:"
  Pop $R1
  ${NSD_CreateText} 0 28u 100% 14u "$INSTDIR"
  Pop $R2
  EnableWindow $R2 0
  ${NSD_CreateLabel} 0 52u 100% 48u "If you chose D:\ or D:\Apps, the installer automatically adds the 小王子 Agent subfolder. Program files stay here; conversations, memories, settings, API keys, sandbox files, and downloads stay under %APPDATA%\小王子 Agent and are removed only if you explicitly choose to clear user data during uninstall."
  Pop $R3
  nsDialogs::Show
FunctionEnd
...
Function BailongmaValidateInstalledPayload
  ...
  IfFileExists "$INSTDIR\小王子 Agent.exe" 0 bailongmaPayloadMissing
  ...
  ClearErrors
  FileOpen $R1 "$INSTDIR\小王子 Agent.exe" r
  ...
  ; Use a conservative lower bound (50 MB) that any complete 小王子 Agent.exe far
  ...
  bailongmaPayloadMissing:
    MessageBox MB_ICONSTOP|MB_OK "小王子 Agent installation did not complete correctly. To avoid leaving a broken app on this computer, setup will stop now.$\r$\n$\r$\nPlease close 小王子 Agent and run this installer again. If the problem continues, send this path to support:$\r$\n$INSTDIR"
    Abort
FunctionEnd
...
!macro customInstall
  ...
  ${if} $installMode == "all"
    WriteRegStr SHELL_CONTEXT "${UNINSTALL_REGISTRY_KEY}" "UninstallString" '"$INSTDIR\Uninstall 小王子 Agent.exe" /allusers --keep-shortcuts'
    WriteRegStr SHELL_CONTEXT "${UNINSTALL_REGISTRY_KEY}" "QuietUninstallString" '"$INSTDIR\Uninstall 小王子 Agent.exe" /allusers /S --keep-shortcuts'
  ${else}
    WriteRegStr SHELL_CONTEXT "${UNINSTALL_REGISTRY_KEY}" "UninstallString" '"$INSTDIR\Uninstall 小王子 Agent.exe" /currentuser --keep-shortcuts'
    WriteRegStr SHELL_CONTEXT "${UNINSTALL_REGISTRY_KEY}" "QuietUninstallString" '"$INSTDIR\Uninstall 小王子 Agent.exe" /currentuser /S --keep-shortcuts'
  ${endIf}
!macroend
...
!macro customInit
  ...
  ${if} $R0 != "小王子 Agent"
    ...
    ${orIf} ${FileExists} "$INSTDIR\resources\app.asar"
      MessageBox MB_ICONSTOP|MB_OK "小王子 Agent is installed in an unsafe shared folder:$\r$\n$\r$\n$INSTDIR$\r$\n$\r$\nTo protect other software, this installer will not continue. Please contact support or manually move/remove only the 小王子 Agent files, then install again."
      Abort
    ${else}
      Call BailongmaNormalizeInstallDir
    ${endIf}
...
!macro customRemoveFiles
  ...
  ; subdirectories 小王子 Agent owns, then remove parent folders only if empty.
  ${if} ${isUpdated}
    ...
    ${if} $R0 != 0
      DetailPrint "小王子 Agent file is busy, aborting upgrade: $R0"
      Push ""
      Call un.restoreFiles
      Pop $R0
      Abort `Can't safely update 小王子 Agent because "$INSTDIR" contains a busy file.`
    ${endif}

    Goto bailongmaRemoveFilesDone
  ${endif}

  Delete "$INSTDIR\小王子 Agent.exe"
  ...
  Delete "$INSTDIR\Uninstall 小王子 Agent.exe"
