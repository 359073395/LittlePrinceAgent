...
import { cloudCapabilityEnabled, runtimeModeSummary } from './runtime-mode.js'
...
console.log('[runtime] mode summary:', runtimeModeSummary())
...
let geoResult = null
if (cloudCapabilityEnabled('GEO_WEATHER')) {
  reportStartupProgress('geo', 'running', '读取缓存或请求实时天气', '正在刷新天气位置')
  geoResult = await withStartupTimeout(collectGeoWeather(), 12000, '[startup] geo-weather')
  reportStartupProgress('geo', 'done', '天气位置已刷新', '天气位置已刷新')
} else {
  reportStartupProgress('geo', 'done', '云端模式已跳过本机天气定位', '云端模式已跳过本机天气定位')
}
...
if (cloudCapabilityEnabled('LOCAL_AGENT_SCAN')) {
  ...
  reportStartupProgress('agents', 'done', '云端模式已跳过本地 Agent 扫描', '云端模式已跳过本地 Agent 扫描')
}
