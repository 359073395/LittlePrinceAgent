...
import { handleDownloadRoutes } from './api/routes/downloads.js'
...
function hasValidAuthToken(req, url) {
  ...
  return timingSafeTokenEqual(bearer, expected)
    || timingSafeTokenEqual(queryToken, expected)
    || timingSafeTokenEqual(cookieToken, expected)
}
...
function attachWebSocketUpgrades(server, port, { sceneWss, cloudWss }) {
  ...
  server.on('upgrade', (req, socket, head) => {
    ...
    const auth = authorizeWebSocketUpgrade(req, {
      ...
      publicOrigin: process.env.LITTLE_PRINCE_AGENT_PUBLIC_URL || '',
      knownPaths,
    })
...
export function startAPI(port = 3721, { getStateSnapshot = null, onActivated = null } = {}) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      const queryToken = url.searchParams.get('token')
      if (queryToken && timingSafeTokenEqual(queryToken, getAuthToken())) {
        const secure = /^(1|true|yes|on)$/i.test(String(process.env.LITTLE_PRINCE_AGENT_SECURE_COOKIE || '').trim())
        res.setHeader('Set-Cookie', `lp_agent_token=${encodeURIComponent(queryToken)}; Path=/; HttpOnly; SameSite=Lax; Max-Age=2592000${secure ? '; Secure' : ''}`)
      }

      if (await handleDownloadRoutes(req, res, url)) return
