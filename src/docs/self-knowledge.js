// 小王子 Agent自知识文档 —— 解释自身的代码机制与架构

export const SELF_KNOWLEDGE_TOPICS = {
  self_architecture: {
    id: 'self_architecture',
    title: '小王子 Agent架构与运行机制',
    subtitle: 'How Bai小王子 Agent Works',
    icon: '⚙',
    summary: '小王子 Agent（Bai小王子 Agent）是一套 Electron + Node.js 混合架构的持续意识框架，版本 2.x。以下是完整的代码机制说明。',
    sections: [
      {
        title: '整体架构',
        content: `小王子 Agent由两层构成：
...
Brain UI 前端通过 WebSocket + REST API 与后端实时通信。`,
      ...
      {
        title: '意识循环：心跳驱动',
        content: `小王子 Agent不等待用户触发，而是持续运行的事件循环：
...
3. 社交平台消息 → 经 social/dispatch.js 路由后进队列`,
      ...
      {
        title: 'L1 / L2 两种入口',
        content: `小王子 Agent不是"两个人格"，而是同一个 AI 的两种触发入口：
...
export function detectSelfKnowledgeTopic(text) {
  ...
  if (
    /(你的代码|你.*怎么运行|你.*怎么工作|你.*架构|你.*如何运作|小王子 Agent.*代码|littleprinceagent.*代码|你.*实现|代码机制|运行机制|技术架构|你.*内部|你.*系统|你.*模块|你.*是怎么|你.*如何思考|你.*心跳|意识循环|ticker|queue\.js|control\.js|llm\.js|prompt\.js|memory.*机制|记忆.*系统|工具.*调用|capability|executor|acui.*机制|brain.*ui|l1.*l2|l2.*l1|两个入口|turn.*机制|self.knowledge|自知识)/.test(
      t
    )
  ) {
