...
function mockRequest(remoteAddress, headers = {}, url = '/scene') {
  return { socket: { remoteAddress }, headers, url }
}

function authorize(pathname, remoteAddress, headers = {}, options = {}) {
  return authorizeWebSocketUpgrade(mockRequest(remoteAddress, headers, options.url || pathname), {
    pathname,
...
async function main() {
  ...
  try {
    ...
    await test('public cloud origin accepts Cookie and query Token credentials', () => {
      const publicOrigin = 'https://agent.example'
      assert.equal(authorize('/scene', '127.0.0.1', { origin: publicOrigin }, { publicOrigin }).ok, true)
      assert.equal(authorize('/scene', '203.0.113.20', {
        origin: publicOrigin,
        cookie: `lp_agent_token=${encodeURIComponent(TEST_TOKEN)}`,
      }, { publicOrigin, lanEnabled: false }).ok, true)
      assert.equal(authorize('/voice/cloud', '203.0.113.20', {
        origin: publicOrigin,
      }, { publicOrigin, lanEnabled: false, url: `/voice/cloud?token=${encodeURIComponent(TEST_TOKEN)}` }).ok, true)
      assert.equal(authorize('/scene', '203.0.113.20', { origin: publicOrigin }, {
        publicOrigin,
        lanEnabled: false,
      }).ok, false)
    })
