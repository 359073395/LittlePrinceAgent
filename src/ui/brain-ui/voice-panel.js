...
export function initVoicePanel({
  ...
  const SILENCE_SEND_MS = (() => {
    const v = parseInt(localStorage.getItem('littleprinceagent-voice-silence-ms') || '', 10);
  ...
  window.addEventListener('littleprinceagent:video-mode', (event) => {
    if (event.detail?.active) {
      window.littleprinceagentVoice.suspendForMedia();
    } else {
      window.littleprinceagentVoice.resumeAfterMedia();
    }
  });

  window.addEventListener('littleprinceagent:music-mode', (event) => {
    if (event.detail?.active) {
      window.littleprinceagentVoice.suspendForMedia();
    } else {
      window.littleprinceagentVoice.resumeAfterMedia();
  ...
  window.addEventListener('littleprinceagent:voice-threshold', (event) => {
